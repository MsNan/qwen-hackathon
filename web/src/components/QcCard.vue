...
function verdictClass(v) {
  return v === '过' ? 'pass' : v === '拒' ? 'reject' : 'revise';
}
</script>
...
<template>
  <div class="qc" v-if="last">
    <!-- 评分（含重写前后对比） -->
    <div class="score-row">
      <div v-if="hasRewrite" class="score before">
        <div class="num">{{ first.score }}</div><div class="lbl">初检</div>
      </div>
      <div v-if="hasRewrite" class="arrow" :class="{ up: delta > 0 }">→<span v-if="delta">{{ delta > 0 ? ' +' + delta : ' ' + delta }}</span></div>
      <div class="score now" :class="verdictClass(last.verdict)">
        <div class="num">{{ last.score }}</div>
        <div class="lbl">{{ hasRewrite ? '复检' : '评分' }}</div>
      </div>
